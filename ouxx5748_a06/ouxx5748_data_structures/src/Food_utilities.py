...
def get_food():
    ...
    name = input("Name: ")
    origin = int(input(f"Origin (Number)\n{Food.origins()}: "))
...
def food_table(foods):
    ...
    headings = ["Food",  "Origin",  "Vegetarian",  "Calories"]

    print(
        f"{headings[0]:<35} {headings[1]:<12} {headings[2]:<10} {headings[3]:<8}")
    ...
    for i in foods:
        if (i.origin == origin or origin == -1):
            if (i.calories <= max_cals or max_cals == 0):
                if (is_veg == False or i.is_vegetarian == True):
                    result.append(i)
