...
class BST:
    ...
    def _remove_aux(self, node, key):
        ...
        if node is None:
            ...
            if node._left is None and node._right is None:
                ...
                replacement_node = self._min_node(node._right)
                node._value = replacement_node._value
                # Remove the replacement node from the right subtree.
                node._right, _ = self._remove_aux(node._right, replacement_node._value)
    ...
    def _min_node(self, node):
        """
        -------------------------------------------------------
        Finds the node with the minimum value in a BST subtree.
        Use: min_node = self._min_node(node)
        -------------------------------------------------------
        Parameters:
            node - the root of the subtree to search (_BST_Node)
        Returns:
            min_node - the node with the minimum value (_BST_Node)
        -------------------------------------------------------
        """
        
        min_node = node
        
        while min_node._left is not None:
    ...
    def _eq_aux(self, source_node, target_node):
        ...
        if source_node is None and target_node is None:
            ...
            equals = (
                source_node._value == target_node._value and
                self._eq_aux(source_node._left, target_node._left) and
                self._eq_aux(source_node._right, target_node._right)
            )
        else:
            equals = False
    ...
    def parent_r(self, key):
        ...
        value = self._parent_r_aux(self._root, None, key)
    ...
    def _parent_r_aux(self, node, parent, key):
        ...
        Use: v = self._parent_r_aux(node, parent of node, key)
        ---------------------------------------------------------
        Parameters:
            node - a BST node
        ...
        if node is None:
            ...
            value = None if parent is None else parent._value
    ...
    def _leaf_count_aux(self, node):
        ...
        if node is not None:
            if node._left is None and node._right is None:
                ...
                left_count = self._leaf_count_aux(node._left)
                right_count = self._leaf_count_aux(node._right)
    ...
    def _one_child_count_aux(self, node):
        ...
        if node is not None:
            if (node._left is None and node._right is not None) or \
               (node._left is not None and node._right is None):
                count = 1
    ...
    def _node_counts_aux(self, node):
        ...
        if node is not None:
            ...
            if node._left is None and node._right is None:
                zero += 1
            elif (node._left is None) != (node._right is None):
                one += 1
            else:
                two += 1
    ...
    def is_balanced(self):
        ...
        balanced = self._is_balanced_aux(self._root) != -1
    ...
    def _is_balanced_aux(self, node):
        ...
        if node is not None:
            left_height = self._is_balanced_aux(node._left)
            right_height = self._is_balanced_aux(node._right)
    ...
    def _is_valid_aux(self, node, min_node, max_node):
        ...
        if node is not None:
            if (min_node is not None and node._value <= min_node._value) or \
               (max_node is not None and node._value >= max_node._value):
                valid = False
            else:
                left_valid = self._is_valid_aux(node._left, min_node, node)
                right_valid = self._is_valid_aux(node._right, node, max_node)
