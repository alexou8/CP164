...
class Deque:
    ...
    def __eq__(self, target):
        ...
        if self._count != target._count:
            ...
            while curr_self is not None and equals:
    ...
    def _swap(self, l, r):
        ...
        if l is not r:
            # Handle neighbors
            if l._next is r:
                # Adjust links for adjacent nodes (l -> r)
                l._next = r._next
                r._prev = l._prev
                if r._next is not None:
                    r._next._prev = l
                else:
                    self._rear = l  # Update rear if needed
                if l._prev is not None:
                    l._prev._next = r
                else:
                    self._front = r  # Update front if needed
                r._next = l
                l._prev = r
            elif r._next is l:
                # Adjust links for adjacent nodes (r -> l)
                r._next = l._next
                l._prev = r._prev
                if l._next is not None:
                    l._next._prev = r
                else:
                    self._rear = r  # Update rear if needed
                if r._prev is not None:
                    r._prev._next = l
                else:
                    self._front = l  # Update front if needed
                l._next = r
                r._prev = l
            else:
                # Adjust links for non-adjacent nodes
                l_prev, l_next = l._prev, l._next
                r_prev, r_next = r._prev, r._next
                ...
                # Final swap of l and r pointers
