...
def create_randoms():
    ...
    for _ in range(TESTS):
        random_list = List()
        for _ in range(SIZE):
            random_list.append(Number(random.randint(0, XRANGE)))
...
def test_sort(title, func):
    ...
    func(values)
    in_order_total = Number.comparisons
    ...
    func(values)
    reversed_total = Number.comparisons
    ...
    random_total = total_comparisons // TESTS
    random_swaps = total_swaps // TESTS

    print(f"{title:14} {in_order_total:>8} {reversed_total:>8} {random_total:>8} "
          f"{in_order_swaps:>8} {reversed_swaps:>8} {random_swaps:>8}")
